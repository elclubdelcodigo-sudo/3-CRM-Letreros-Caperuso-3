...
const oldGenerateFn = appJs.match(/async function generateIAPageMessage\(\) \{[\s\S]*?finally \{\s*if \(btn\) btn\.disabled = false;\s*\}\s*\}/)[0];
...
const newGenerateFn = `async function generateIAPageMessage() {
    ...
    try {
        ...
        else if (msgType === "soporte") typeText = "un mensaje de soporte técnico o amable";
        ...
        const prompt = \`Genera \${typeText} para enviarlo por \${channel} a \${recipient}. \${qData}. Escribe únicamente el texto del mensaje, directo al grano, simple y muy corto, sin texto adicional.\`;
        ...
        const response = await fetch('/api/gemini/generate', {
            ...
            body: JSON.stringify({ 
                prompt,
                systemInstruction: "Eres un asistente de ventas de Letreros Caperuso. Crea respuestas simples, muy simples y ligeras. No seas repetitivo. Solo el contenido del mensaje."
            })
        });
        ...
        if (logArea) logArea.innerHTML += "<br>[SYS] <span style='color:#ff4444'>Error: " + err.message + "</span>";
    } finally {
...
// Find where 'mensajeria-ia' navigation is handled in navigateToPage()
const targetNavigate = `        case 'mensajeria-ia':
            // Logic for IA page
            break;`;

if (appJs.includes(targetNavigate)) {
    const newNavigate = `        case 'mensajeria-ia':
            populateIAQuotationSelect();
            break;`;
    appJs = appJs.replace(targetNavigate, newNavigate);
} else {
    // If not found, let's see if there's an empty case or what
    // Just inject populateIAQuotationSelect() into navigateToPage()
}
...
console.log('IA logic patched successfully');
